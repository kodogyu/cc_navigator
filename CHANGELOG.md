...
## 0.2.1 — branch `cc_navigator_chan`

- **상태 메시지 자동 삭제**: 답장/이동 결과 같은 일시 메시지가 10초 뒤 자동으로 사라짐
  (예: "VS Code 세션은 답장을 지원하지 않습니다"가 계속 남던 문제).
- **멈춘 "작업 중" 세션 처리**: VSCode 세션은 idle이어도 프로세스가 살아있어 행이 남는데,
  마지막 turn의 `Stop` 훅을 놓치면 상태가 "작업 중"에 고착됐음. 이제 **15분 이상 갱신 없는
  "작업 중"은 유휴로 표시** (긴 단일 작업 오탐 최소화, `STALE_WORKING_SECONDS`로 조정).
- **클릭하면 바로 세션 이동** 설정 추가: 켜면 세션 행을 한 번 클릭하면 펼치지 않고 곧바로
  세션 창으로 이동.
